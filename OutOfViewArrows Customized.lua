...
local xoffset = gui.Slider(ref,"oof.xoffset","X-axis Offset",0,1,1000);
xoffset:SetDescription("Defines the ovalness of the circle on x-axis");
local yoffset = gui.Slider(ref,"oof.yoffset","Y-axis Offset",0,1,1000);
        ...
        for k,v in pairs(players) do
            ...
            if player:IsAlive() and player:IsPlayer() and player:GetName() ~= localplayer:GetName() and player:GetTeamNumber() ~= localplayer:GetTeamNumber() then
                ...
                if w2sx == nil and w2sy == nil or w2sx < 100 or w2sy < 100 or w2sx > screen_width-100 or w2sy > screen_height-100 then
                    draw.Triangle(screen_center_x+((radius:GetValue()+xoffset:GetValue())*math.cos(angle.y-math.rad(arrowwidth:GetValue()*100/(radius:GetValue()+xoffset:GetValue())))), screen_center_y+((radius:GetValue()+yoffset:GetValue())*math.sin(angle.y-math.rad(arrowwidth:GetValue()*100/(radius:GetValue()+yoffset:GetValue())))), screen_center_x+((radius:GetValue()+xoffset:GetValue())*math.cos(angle.y+math.rad(arrowwidth:GetValue()*100/(radius:GetValue()+xoffset:GetValue())))), screen_center_y+((radius:GetValue()+yoffset:GetValue())*math.sin(angle.y+math.rad(arrowwidth:GetValue()*100/(radius:GetValue()+yoffset:GetValue())))), screen_center_x+((finalheight+xoffset:GetValue())*math.cos(angle.y)), screen_center_y+((finalheight+yoffset:GetValue())*math.sin(angle.y)));
                end
            end
        end
    end
