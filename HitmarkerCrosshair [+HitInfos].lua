...
local function onDraw()
   ...
   for i, j in pairs(urmom_gay) do       
       if globals.RealTime() > j[1] + time_until_each_fucking_thing_disappears then
           ...
           draw.Text(850, amount_of_things_currently_on_the_screen_lol * number_of_little_squares_between_each_line + 535, string.format( "Hit %s for %s (%s hp)",
           j[2], j[4], j[5])) -- j[3]
           draw.TextShadow(850, amount_of_things_currently_on_the_screen_lol * number_of_little_squares_between_each_line + 535, string.format( "Hit %s for %s (%s hp)",
           j[2],  j[4], j[5])) --j[3],


           draw.Text(850, amount_of_things_currently_on_the_screen_lol * number_of_little_squares_between_each_line + 535, string.format( "- %s  %s",
            j[4], j[3]))
           draw.TextShadow(850, amount_of_things_currently_on_the_screen_lol * number_of_little_squares_between_each_line + 535, string.format( "- %s  %s",
           j[4], j[3]))
