...
local b_pressed = input.IsButtonPressed
local b_released = input.IsButtonReleased
local y_offset = gui.Slider(gui.Reference("Visuals","Local","Helper"), "y.ind.offset", "Override Damage Indicators Y-Offset", -70, -450, 450)
local rage_ref_extra = gui.Reference("RAGEBOT", "ACCURACY", "WEAPON");
local changeDamageText = gui.Text(rage_ref_extra, "--- Changer Damage ---");
local newDamage = gui.Slider(rage_ref_extra, "NewDamage", "Min damage", 0, 1, 100);
local changeKey = gui.Keybox(rage_ref_extra, "ChangeDmgKey", "Change damage key", nil);
local firstTime = 1;
...
function changeDmgMain()
    ...
    if (b_pressed(changeKey:GetValue())) then
        if (firstTime == 1) then
    ...
    elseif (b_down(changeKey:GetValue())) then
        -- do nothing ;)
    elseif (b_released(changeKey:GetValue())) then
