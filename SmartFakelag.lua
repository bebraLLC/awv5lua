...
callbacks.Register( 'Draw',  function()
	...
	if MASTER_SWITCH:GetValue() and FAKELAG_SMART_MODE:GetValue() then

		SMARTMODE_GROUPBOX:SetDisabled(false)
		SMARTMODE_GROUPBOX:SetInvisible(false)

	elseif not MASTER_SWITCH:GetValue() and not FAKELAG_SMART_MODE:GetValue() then

		SMARTMODE_GROUPBOX:SetDisabled(true)
		SMARTMODE_GROUPBOX:SetInvisible(true)	
	
	elseif MASTER_SWITCH:GetValue() and not FAKELAG_SMART_MODE:GetValue() then

		SMARTMODE_GROUPBOX:SetDisabled(true)
		SMARTMODE_GROUPBOX:SetInvisible(true)		

	elseif not MASTER_SWITCH:GetValue() and FAKELAG_SMART_MODE:GetValue() then
...
local function FakelagOnDoubletap()
	...
	if GetValue( "rbot.accuracy.weapon.asniper.doublefire" ) >= 0 then
		Doubletap_On = true
	end
...
local function FakelagSmartMode()
	...
	if Standing and Time + 0.2 < globals.CurTime() then
		if ( FAKELAG_STANDING == 0 ) or
	   	...
	   	( FAKELAG_EXTRA:GetValue() and FAKELAG_ON_DOUBLETAP:GetValue() and GetValue("rbot.accuracy.weapon.asniper.doublefire") >= 1 ) then
			SetValue( "misc.fakelag.enable", 0 );
		else
			SetValue( "misc.fakelag.enable", 1 );
		end
