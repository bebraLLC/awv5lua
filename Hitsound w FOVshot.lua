...
local sliderEnd = gui.Slider(msc_ref, 'lua_healthshot_fovmarkerr_endfov', 'End FOV', 120, 50, 150)
...
local function showFov(count, player)
  ...
  if globals.RealTime() < player.delay then
      if sliderStart:GetValue() > nd then
        if player.fov > nd then player.fov = player.fov - (nd + player.fov) * sliderSmooth:GetValue()/20000 end -- время анимации
